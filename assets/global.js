	...
	updateMedia(mediaId) {
		...
		swiperWrappers.forEach((elem) => {
			if (!this.isHidden(elem)) {
				...
				const mediaSlider = elem.querySelector('.js-media-list')?.swiper
				const mediaIndex = mediaSlider?.slides.indexOf(newMedia)

				if (mediaSlider && mediaIndex >= 0) {
					mediaSlider.slideTo(mediaIndex)
				}
			}
		})
	}
